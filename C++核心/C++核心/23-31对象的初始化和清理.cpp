...
//当一个类中的成员是另一个类的对象时，称该成员为对象成员
//在实例化该类时，会先实例化那个对象成员（先调用其构造函数）
//销毁时，则是先释放该类的实例化对象，再释放那个对象成员（后调用其析构函数）
...
public:
	static int s; // 静态成员变量，类的所有对象都共用同一数据，在程序运行前分配内存

	// 静态成员函数：类的所有对象共享，只能访问静态成员变量！！
	static void test(int n) {
		//a = n; 错误，无法访问非静态,因为无法区分是哪个变量的
		s = n;
		ss = n;
	}
};
...
int Test::s = 66666; //必须在类外初始化（全局域下）
int Test::ss = 77777; //必须在类外初始化（全局域下）
